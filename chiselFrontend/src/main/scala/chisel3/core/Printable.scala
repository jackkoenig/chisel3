...
private[chisel3] trait PrintableAPI {
  ...
  /** Name of the resuling Verilog wire
    * Currently implemented with %I identifier in Firrtl
    */
  case class VerilogName(data: Data) extends Printable {
    final def unpack(ctx: Component): (String, Iterable[String]) =
      ("%I", List(data.ref.fullName(ctx)))
  }
  /** Put innermost name (eg. field of bundle) */
  case class Name(data: Data) extends Printable {
    final def unpack(ctx: Component): (String, Iterable[String]) =
      (data.ref.name, List.empty)
  }
  ...
  /** Name of the resulting Verilog Module
    * Currently implemented with %m specifier in Firrtl
    */
  case object ModuleName extends Printable {
    final def unpack(ctx: Component): (String, Iterable[String]) = ("%m", List.empty)
  }
